...
def process (f):
  ...
  for line in f:
    ...
    if not m:
      print ("%s:%d: unrecognised type of option: %s" % (f.name, iline, line), file=sys.stderr)
      continue
    o, v = m.groups()
...
#      cfg[o] = v
    ...
    if vol is None:
      print ("%s:%d: unrecognised global option: %s" % (f.name, iline, line), file=sys.stderr)
      continue
    ...
    if not m:
      print ("%s:%d: unrecognised option: %s" % (f.name, iline, line), file=sys.stderr)
      continue
