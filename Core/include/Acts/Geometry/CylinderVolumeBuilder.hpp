...
struct WrappingConfig {
 public:
  ...
  static const Logger& logger() {
    static std::unique_ptr<const Logger> logger = getDefaultLogger("WrappingConfig", Logging::INFO);
    return *logger;
  }
      ...
      if (!cVolumeConfig) {
        ...
        if (existingVolumeConfig.rMax < containerVolumeConfig.rMin) {
          ...
          ACTS_WARNING(
              ...
              << "\ncontainerVolumeConfig, r = " << containerVolumeConfig.rMin << " - " << containerVolumeConfig.rMax);
        }
